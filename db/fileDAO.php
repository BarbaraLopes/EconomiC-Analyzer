...
class fileDAO
{
    ...
    public function remover($file)
    {
        global $pdo;
        try {
            $statementS = $pdo->prepare("SELECT id_payment FROM tb_payments WHERE tb_files_id_file = :id");
            $statementS->bindValue(":id", $file->getIdFile());
            $statementS->execute();
    ...
    public function salvar($file)
    {
        ...
        try {
            ...
            $statement->bindValue(":str_name_file", $file->getStrNameFile());
            $statement->bindValue(":str_month", $file->getStrMonth());
            $statement->bindValue(":str_year", $file->getStrYear());
    ...
    public function atualizar($file)
    {
        ...
        try {
            ...
            if ($statement->execute()) {
                ...
                $file->setStrNameFile($rs->str_name_file);
                $file->setStrMonth($rs->str_month);
                $file->setStrYear($rs->str_year);
                return $file;
            } else {
                throw new PDOException("Erro: Não foi possível executar a declaração sql");
            }
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_file, str_name_file, str_month, str_year FROM tb_files order by id_file LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
