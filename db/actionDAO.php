...
class actionDAO
{
    ...
    public function remover($action)
    {
        global $pdo;
        try {
            $statementS = $pdo->prepare("SELECT id_payment FROM tb_payments WHERE tb_action_id_action = :id");
            $statementS->bindValue(":id", $action->getIdAction());
            $statementS->execute();
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_action, str_cod_action, str_name_action FROM tb_action order by id_action LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
     ...
     <tbody>";
        ...
        <td><a href='?act=upd&id=$var->id_action'><i class='ti-reload'></i></a></td>
        <td><a href='?act=del&id=$var->id_action'><i class='ti-close'></i></a></td>
       </tr>";
            endforeach;
            echo "
</tbody>
