...
class subfunctionsDAO
{
    ...
    public function remover($subfunction)
    {
        global $pdo;
        try {
            $statementS = $pdo->prepare("SELECT id_payment FROM tb_payments WHERE tb_subfunctions_id_subfunction = :id");
            $statementS->bindValue(":id", $subfunction->getIdSubfunction());
            $statementS->execute();
            ...
            $statement->bindValue(":id", $subfunction->getIdSubfunction());
    ...
    public function salvar($subfunction)
    {
        ...
        try {
            ...
            $statement->bindValue(":str_cod_subfunction", $subfunction->getStrCodSubfunction());
            $statement->bindValue(":str_name_subfunction", $subfunction->getStrNameSubfunction());
    ...
    public function atualizar($subfunction)
    {
        ...
        try {
            ...
            if ($statement->execute()) {
                ...
                $subfunction->setStrCodSubfunction($rs->str_cod_subfunction);
                $subfunction->setStrNameSubfunction($rs->str_name_subfunction);
                return $subfunction;
            } else {
                throw new PDOException("Erro: Não foi possível executar a declaração sql");
            }
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT id_subfunction, str_cod_subfunction, str_name_subfunction FROM tb_subfunctions order by id_subfunction LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
     ...
     <tbody>";
            foreach ($dados as $var):
                echo "<tr>
        <td>$var->id_subfunction</td>
        <td>$var->str_cod_subfunction</td>
        <td>$var->str_name_subfunction</td>
        <td><a href='?act=upd&id=$var->id_subfunction'><i class='ti-reload'></i></a></td>
        <td><a href='?act=del&id=$var->id_subfunction'><i class='ti-close'></i></a></td>
       </tr>";
            endforeach;
            echo "
</tbody>
